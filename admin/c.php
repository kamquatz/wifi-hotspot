                        ...
                        <div class="col-sm-8">
                            <div class="card">
                                <div class="card-block">
                                    ...
                                    <br />
                                    <a href="javascript:window.location.reload();" class="btn btn-sm btn-success waves-effect waves-light pull-right">Refresh</a>
                                        ...
                                        <table id="alt-pg-dt" class="table table-striped table-bordered nowrap" style="font-size: smaller">
                                                   <thead>
                                                      <tr>
                                                            ...
                                                            <th class="text-center">Videos<br />Watched</th>
                                                      </tr>
                                                   </thead>
                                                   <tbody>
<?php 
...
if (isset($_REQUEST["customer_filter"])) {    
    switch($customer_filter){
        case 'today': $filter = ' WHERE DATE(`expiry_time`) = DATE(CURRENT_DATE()) '; break;
        case 'month': $filter = ' WHERE MONTH(`expiry_time`) = MONTH(CURRENT_DATE()) AND YEAR(`expiry_time`) = YEAR(CURRENT_DATE()) '; break;
        case 'active': $filter = ' WHERE `device` = 1 '; break;
    ...
    $sql = 
        'SELECT id, phone, station, device, connections, updated_at, expiry_time, NOW()
        FROM `'.$_SESSION['logged_user'].'` '.$filter.' ORDER BY `device` ASC, `connections` DESC';
    $counter = $ttl_amt = $ttl_active = 0;
    if ($stmt = $mysqli->prepare($sql)) {
          $stmt->execute();
          $stmt->bind_result($id, $phone, $station, $device, $connections, $created_at, $expiry_time, $now);
          while ($stmt->fetch()) { 
              $counter++;
              switch($device){
                  case 0 : {
...
<?php  }
          ...
          ?>
                                ...
                                <div class="card-block">
<?php 
$sql2 = 'SELECT id, name FROM `sites` ORDER BY `name` ASC';
if ($stmt = $mysqli->prepare($sql2)) {
      $stmt->execute();
      $stmt->bind_result($id, $name);
      while ($stmt->fetch()) { 
          $btn = ($name==$_SESSION['logged_user']) ? 'btn-primary' : 'btn-danger';
          $href = ($name==$_SESSION['logged_user']) ? 'c.php' : 'logout.php?site='.$name;
?>
                                    <a href="<?php echo $href; ?>" class="btn btn-sm <?php echo $btn; ?> waves-effect waves-light">
